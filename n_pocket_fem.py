for i in range(100):
	...
	box_obj.Width = 300
	box_obj.Length = 1000
	box1_obj = doc.addObject('Part::Box', 'Box1')
	box1_obj.Height = 50
	import random
	box_width = random.randint(10, 60)
	box1_obj.Width = box_width
	box_length = random.randint(10,60)
	box1_obj.Length = box_length 
	box_placement_x = random.randint(62, 240)
	box_placement_y = random.randint(62, 240)
	# cyl_obj.Placement = App.Placement(App.Vector(300,0,0),App.Rotation(App.Vector(0,0,1),0))
	box1_obj.Placement = App.Placement(App.Vector(box_placement_x,box_placement_y,-5),App.Rotation(App.Vector(0,0,1),0))
	# cyl_obj.Placement =
	cut_obj = doc.addObject("Part::Cut","Cut")
	cut_obj.Base = doc.Box
	cut_obj.Tool = doc.Box1
